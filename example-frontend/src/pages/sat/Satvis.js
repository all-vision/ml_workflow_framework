  ...
  render() {
    ...
    return (
        ...
        <Autocomplete
          ...
          options={
            this.state.selectedClusters.length > 0
              ? this.state.filteredStations.filter((station) => !(station.orbitMinutes > 0))
              : this.state.stations
          }
          ...
          <FormControl style={{ maxWidth: 150, minWidth: 150 }}>
            ...
            >
              {this.state.modelsNames.map((model, index) => {
                return (
                  <MenuItem>{model.name}</MenuItem>
                )
              })}
            </Select>
          </FormControl>
          <FormControl style={{ maxWidth: 150, minWidth: 150, marginLeft: '2vw' }}>
            <InputLabel id="demo-simple-select-label">
            ...
            >
              ...
              {this.state.clusters.map((cluster, index) => {
                ...
                return (
                  <MenuItem value={cluster} key={index} style={{ color: color_palette[cluster], fontWeight: '900'}}>
                    {cluster}
                  </MenuItem>
                );
              })}
      ...
      <Button
            ...
            variant="contained"
            color="primary"
            onClick={() => this.handleSubmitXYValues(this.state.selectedYValue, this.state.selectedXValue)}
          >
